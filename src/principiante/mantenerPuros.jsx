export default function Mantener() {
  return (
    ...
    <p>
    Este componente está leyendo y escribiendo una variable <span className="note">guest</span> declarada fuera de ella. Esto significa que <b>llamar a este componente varias veces producirá diferente JSX!</b> Y lo que es más, si otros componentes leen <span className="note">guest</span>, también producirán diferente JSX, ¡dependiendo de cuándo se procesaron! Eso no es predecible.
    </p>
    <p>
    ...
    <p>
    Puedes arreglar este componente <span className="note">pasando guest como prop en su lugar</span>:
    </p>
    <pre className="ide">
    <code>
			<span className="blue">function</span>{" "}
    ...
    <p>Ahora tu componente ya es puro, ya que el JSX que devuelve solo depende de la prop <span className="shadow">guest</span>.</p>
    <p>
    ...
    <p>
    Aunque es posible que aún no los hayas usado todos, en React hay tres tipos de entradas que puedes leer mientras se renderiza: <span className="note">props</span>, <span className="note">state</span>, y <span className="note">context</span>. Siempre debes tratar estas entradas como solo lectura.
    </p>
    <p>
    Cuando quieras cambiar algo en respuesta a la entrada del usuario, debes <span className="note">asignar el estado</span> en lugar de reescribir la variable. Nunca debes cambiar variables u objetos preexistentes mientras tu componente está renderizando.
    </p>
    <p>
    React ofrece un «Modo estricto» en el que llama a la función de cada componente dos veces durante el desarrollo. <b>Al llamar a las funciones del componente dos veces, el modo estricto ayuda a encontrar componentes que rompan estas reglas</b>.
    </p>
    <p>
    Observa cómo el ejemplo original mostraba «Guest #2», «Guest #4», y «Guest #6» en lugar de «Guest #1», «Guest #2», y «Guest #3». La función original era impura, por lo que al llamarla dos veces se rompió. Pero la versión corregida funciona sin importar que la función sea llamada dos veces cada vez. <b>Las funciones puras solo se calculan, por lo que llamarlas dos veces no cambiará nada</b> —como llamar <span className="shadow">double(2)</span> dos veces no cambia lo que se devuelve, y retorna y = 2x dos veces no cambia lo que y es. Las mismas entradas, las mismas salidas. Siempre.
    </p>
    <p>
    El modo estricto no tiene ningún efecto en producción, por lo que no ralentizará la aplicación para tus usuarios. Para optar por el modo estricto, puedes envolver tu componente raíz en <span className="shadow">&lt;React.StrictMode&gt;</span>. Algunos frameworks hacen esto por defecto.
    </p>
    ...
    <p>
    Sin embargo, está completamente bien cambiar variables y objetos que acabas de crear mientras renderizas. En este ejemplo, creas un array <span className="shadow">[]</span>, lo asignas a la variable <span className="shadow">cups</span>, y luego haces un <span className="shadow">push</span> con una docena de tazas adentro:
    </p>
    <pre className="ide">
    	<span className="blue">function</span>{" "}
			<span className="yellow">Cup</span>
    ...
    <p>
    ¡Si la variable <span className="shadow">cups</span> o el array <span className="shadow">&#91;&#93;</span> se crearon fuera de la función <span className="shadow">TeaGathering</span>, este sería un gran problema! Estarías cambiando un objeto preexistente haciendo push a ese array.
    </p>
    <p>
    Sin embargo, está bien porque los has creado durante el mismo renderizado, dentro de <span className="shadow">TeaGathering</span>. Ningún código fuera de <span className="shadow">TeaGathering</span> sabrá nunca que esto ha ocurrido. Esto se llama <b>«mutación local»</b> —es como el pequeño secreto de tu componente.
    </p>
    ...
    <p>
    En React, <b>los efectos secundarios generalmente deberían estar dentro de los</b> <span className="note">controladores de eventos</span>. Los controladores de eventos son funciones que React ejecuta cuando realiza alguna acción (por ejemplo, cuando haces clic en un botón). ¡Aunque los controladores de eventos están definidos dentro de tu componente, no corren durante el renderizado! <b>Por lo tanto, los controladores de eventos no necesitan ser puros</b>.
    </p>
    <p>
    Si has agotado todas las demás opciones y no puedes encontrar el controlador de evento adecuado para tu efecto secundario, aún puedes adjuntarlo en el retorno del JSX con un llamado a <span className="note">useEffect</span> en tu componente. Esto le dice a React que lo ejecute más tarde, después del renderizado, cuando se permiten efectos secundarios. <b>Sin embargo, este enfoque debería ser tu último recurso</b>.
    </p>
    <p>
